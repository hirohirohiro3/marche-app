...
const sections = [
  ...
  },
  {
    ...
    points: [
      ...
      {
        primary: 'メールアドレス（レシート送付用）',
        secondary: '利用者がレシートのメール送信機能を利用する場合、送付先となるメールアドレスを収集します。この情報はレシートの送信のみに使用され、他の目的には使用されません。',
      },
      {
        primary: '決済情報',
        secondary: '有料の注文を行う場合、決済処理のためにクレジットカード情報等が利用されますが、これらの情報は決済代行会社（Stripe）が直接取得・管理し、当方ではクレジットカード情報を一切保持しません。',
      },
      {
  ...
  },
  {
    title: '利用者の権利',
    content: [
      '本サービスでは、お問い合わせフォームおよびレシート送付機能を除き、氏名やメールアドレス等、利用者が直接当方に提供する個人情報は収集しておりません。LocalStorageに保存される顧客識別ID (uid) の削除については、利用者が自身のブラウザ設定から行うことができます。お問い合わせフォームを通じて提供された個人情報の開示、訂正、削除等を希望される場合は、第9条のお問い合わせ窓口までご連絡ください。',
    ],
  },
  {
    title: 'プライバシーポリシーの変更',
